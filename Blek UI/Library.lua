...
function Library:Create(table)
    ...
    local Input = main.InputBegan:Connect(function(Key)
        if Key.UserInputType == Enum.UserInputType.MouseButton1 or Key.UserInputType == Enum.UserInputType.Touch then
            local ObjectPosition = Vector2.new(Key.Position.X - main.AbsolutePosition.X, Key.Position.Y - main.AbsolutePosition.Y)
            local dragging = true
            ...
            moveConnection = RunService.RenderStepped:Connect(function()
                if dragging then
                    ...
                    if Key.UserInputType == Enum.UserInputType.Touch then
                        FrameX, FrameY = Key.Position.X - ObjectPosition.X, Key.Position.Y - ObjectPosition.Y
                    else
                        FrameX, FrameY = Mouse.X - ObjectPosition.X, Mouse.Y - ObjectPosition.Y
                    end
            ...
            local endInput
            endInput = Key.Changed:Connect(function()
