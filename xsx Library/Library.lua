...
local dragging = false -- Tracks if the slider is being dragged
local function UpdateSlider(input)
    local position = math.clamp(input.Position.X - sliderBackground.AbsolutePosition.X, 0, sliderBackground.AbsoluteSize.X)
    TweenService:Create(sliderIndicator, TweenTable["slider_drag"], {Size = UDim2.new(0, position, 0, 12)}):Play()

    ValueNum = math.floor((((values.max - values.min) / sliderBackground.AbsoluteSize.X) * position) + values.min)
